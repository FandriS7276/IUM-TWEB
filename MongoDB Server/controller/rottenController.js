...
exports.getReviewsByType = async (req, res) => {
    try {
        ...
        const snubbed = await rottenReview.aggregate(pipeline);

        const totalPipeline = [
            { $match: { review_type: 'Fresh' } },
            {
                $lookup: {
                    from: "oscarCollection",
        ...
        const total = await rottenReview.aggregate(totalPipeline).then(result => result[0]?.total || 0);
        ...
        res.status(200).json({
            ...
            metadata: {
                fetchedAt: new Date().toISOString(),
                resultCount: snubbed.length,
                appliedFilters: { review_type: 'Fresh', oscar:false }
            }
        });
    }
    catch (err) {
